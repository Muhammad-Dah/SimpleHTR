...
def analyzePixelRelevance():
    ...
    batch = Batch([Constants.gtText], [word_image_preprocess(img, Model.imgSize)])
    ...
    for x in range(w):
        for y in range(h):
            ...
            for g in grayValues:
                ...
                imgChanged[x, y] = g
                imgsMarginalized.append(word_image_preprocess(imgChanged, Model.imgSize))
...
def analyzeTranslationInvariance():
    ...
    for dy in range(Model.imgSize[0] - h + 1):
        ...
        imgList.append(word_image_preprocess(targetImg, Model.imgSize))
