...
class Model:
    ...
    def __init__(self, charList=open('../model/charList.txt').read(), decoderType=DecoderType.BestPath,
                 mustRestore=False, is_line=False, dump=False):
        ...
        imgSize = Model.imgSize if is_line is False else Model.lineImgSize
        # input image batch
        self.inputImgs = tf.compat.v1.placeholder(tf.float32, shape=(None, imgSize[0], imgSize[1]))
    ...
    def inferBatch(self, batch, calcProbability=False, probabilityOfGT=False):
        ...
        batch_imgs = batch.imgs
        if batch_imgs[0].shape == Model.imgSize[::-1]:
            batch_imgs = [cv2.transpose(img) for img in batch_imgs]
        ...
        feedDict = {self.inputImgs: batch_imgs, self.seqLen: [Model.maxTextLen] * numBatchElements,
                    self.is_train: False}
